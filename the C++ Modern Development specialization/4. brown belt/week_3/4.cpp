...
public:
    ...
    WriteAccess operator[](const K& key) {
        return {lock_guard(data[Get_Index(key)].mtx), data[Get_Index(key)].bucket_map[key]};
    }
    ReadAccess At(const K& key) const {
        if (data[Get_Index(key)].bucket_map.count(key)) {
            return {lock_guard(data[Get_Index(key)].mtx), data[Get_Index(key)].bucket_map.at(key)};
        }
        throw std::out_of_range("Value is not present");
    }

    bool Has(const K& key) const {
        lock_guard(data[Get_Index(key)].mtx);
        return data[Get_Index(key)].bucket_map.count(key);
    }
    ...
    MapType BuildOrdinaryMap() const {
        ...
        for (const auto& i: data) {
            i.mtx.lock();
            for (const auto& [key, val]: i.bucket_map) {
                answer.insert({key, val});
            }
            i.mtx.unlock();
        }
        return answer;
...
public:
    ...
    struct WriteAccess {
        WriteAccess(mutex& mut, V& value): mtx(mut), ref_to_value(value) {
            mut.lock();
        }
        ~WriteAccess() {
            mtx.unlock();
    ...
    struct ReadAccess {
        ReadAccess(mutex& mut, const V& value): mtx(mut), ref_to_value(value) {
            mut.lock();
        }
        ~ReadAccess() {
            mtx.unlock();
    ...
    WriteAccess operator[](const K& key) {
        return {data[Get_Index(key)].mtx, data[Get_Index(key)].bucket_map[key]};
    }
    ReadAccess At(const K& key) const {
        if (data[Get_Index(key)].bucket_map.count(key)) {
            return {data[Get_Index(key)].mtx, data[Get_Index(key)].bucket_map.at(key)};
        }
        throw std::out_of_range("Value is not present");
    }
