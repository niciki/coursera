...
template <typename T>
struct T_struct {
    T_struct(T& data, mutex& m): ref_to_value(data), mtx(m) {
        mtx.lock();
    };
    ~T_struct() {
        mtx.unlock();
...
template <typename T>
struct T_struct_const {
    T_struct_const(const T& data, mutex& m): ref_to_value(data), mtx(m) {
        mtx.lock();
    };
    ~T_struct_const() {
        mtx.unlock();
...
public:
    ...
    T_struct<T> GetAccess() {
        return T_struct(value, m);
    }
    T_struct_const<T> GetAccess() const {
        return T_struct_const(value, m);
    }
