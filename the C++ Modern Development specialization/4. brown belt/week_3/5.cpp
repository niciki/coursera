...
private:
    ...
    struct IndexProxy {
        IndexProxy(Polynomial& curr, size_t size): p(curr), idx(size) {}
        IndexProxy& operator= (T data) {
            if (idx >= p.coeffs_.size()) {
                p.coeffs_.resize(idx + 1, 0);
            }
            p.coeffs_[idx] = std::move(data);
            p.Shrink();
            return *this;
        }
        operator T() const {
            return std::as_const(p)[idx];
        }
    private:
        Polynomial& p;
        size_t idx;
