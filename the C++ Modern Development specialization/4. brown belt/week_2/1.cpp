...
vector<Spending> LoadFromXml(istream& input) {
    Document root = Load(input);
    vector<Spending> answ;
    for (auto& i :root.GetRoot().Children()) {
        answ.push_back(Spending{i.AttributeValue<string>("category"),
                    i.AttributeValue<int>("amount")});
    }
    return answ;
}
