...
Json::Document XmlToJson(const Xml::Document& doc) {
    ...
    for (const Xml::Node& n : doc.GetRoot().Children()) {
        ...
        result.emplace_back(std::map<std::string, Json::Node>{
            ...
            {"amount", Json::Node(atoi(n.AttributeValue<std::string>("amount").c_str()))}
        });
    }
...
void TestXmlToJson() {
    ...
    for (size_t i = 0; i < items.size(); ++i) {
        ...
        AssertEqual(item.at("category").AsString(), expected_category[i], feedback_msg + " category");
        AssertEqual(item.at("amount").AsInt(), expected_amount[i], feedback_msg + " amount");
    }
}
