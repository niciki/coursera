...
struct MoneyPair{
    ...
    double spend = 0.0;
    MoneyPair() = default;
    MoneyPair(double gain_, double spend_): gain(gain_), spend(spend_) {}
...
MoneyPair operator+(const MoneyPair& lhs, const MoneyPair& rhs){
	return {lhs.gain + rhs.gain, lhs.spend + rhs.spend};
}
...
public:
    ...
    MoneyPair Collapse(MoneyPair origin, IndexSegment segment) const {
        return {origin.gain * tax_.factor + add_.delta * segment.length(), origin.spend + spend_.delta * segment.length()};
    }
...
class BudgetManager : public SummingSegmentTree<MoneyPair, BulkLinearUpdater> {
public:
