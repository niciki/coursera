...
struct AddressHasher {
    size_t operator() (const Address& adr) const {
        return string_hash(adr.city) * 101 + string_hash(adr.street) + 9369319 * adr.building;
    }
...
struct PersonHasher {
    size_t operator() (const Person& pers) const {
        return string_hash(pers.name) * 433 + pers.height * 524287 + double_hash(pers.weight) + address_hash(pers.address);
    }
    hash<string> string_hash;
