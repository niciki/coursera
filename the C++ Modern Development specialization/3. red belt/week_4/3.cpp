...
public:
    ...
    Items GetItems() const {
        ...
        for (int i = 0; i < data.size(); ++i) {
            items_[i] = {static_cast<TAirport>(i), Get(static_cast<TAirport>(i))};
        }
        return items_;
    }
private:
...
int main() {
    ...
    /*AirportCounter<SmallCountryAirports> s;
    cout << s.GetItems().length();*/
    return 0;
}
...
public:
    ...
    Items GetItems() const {
        set<Item> s;
        for (int i = 0; i < data.size(); ++i) {
            s.insert({static_cast<TAirport>(i), data[i]});
        }
        return s;
    }
private:
